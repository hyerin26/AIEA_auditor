...
def main():
    ...
    total_updates = TOTAL_STEPS // (N_STEPS * NUM_ENVS)
    ...
    while global_step < TOTAL_STEPS:
        ...
        all_advantages = []
        R_temp = next_values.detach() * (1.0 - last_dones)

        for i in reversed(range(len(rewards))):
            reward_i = rewards[i].unsqueeze(1)
            done_i = dones[i].unsqueeze(1)
            R_temp = reward_i + GAMMA * R_temp * (1.0 - done_i)
            all_advantages.append(R_temp - values[i])
        ...
        for i in reversed(range(len(rewards))):
            advantage = all_advantages[len(rewards) - 1 - i]
            normalized_advantage = (advantage - adv_mean) / adv_std

            policy_loss = policy_loss - (log_probs[i].unsqueeze(1) * normalized_advantage.detach()).mean()
