...
DEFAULT_SAVE_DIR = "./train_results/finetuned_a2c_results"
...
def save_training_plots(
    ...
    np.save(os.path.join(save_dir, "entropy_coefs.npy"), np.array(entropy_coefs))
    np.save(os.path.join(save_dir, "learning_rates.npy"), np.array(learning_rates))
    ...
    plt.ylabel("Reward")
    plt.title("Fine-tuned A2C Training Rewards")
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(os.path.join(plot_dir, "finetuned_a2c_training_rewards.png"))
    plt.close()
    ...
    if len(eval_rewards) > 0:
        ...
        plt.title("Fine-tuned A2C Eval Rewards")
        plt.legend()
        plt.grid(True)
        plt.tight_layout()
        plt.savefig(os.path.join(plot_dir, "finetuned_a2c_eval_rewards.png"))
        plt.close()
    ...
    if len(losses) > 0:
        ...
        plt.xlabel("Training Step")
        plt.ylabel("Loss")
        plt.title("Fine-tuned A2C Training Loss")
        plt.legend()
        plt.grid(True)
        plt.tight_layout()
        plt.savefig(os.path.join(plot_dir, "finetuned_a2c_training_loss.png"))
        plt.close()
    ...
    if len(policy_losses) > 0:
        ...
        plt.title("Fine-tuned A2C Policy Loss")
        plt.legend()
        plt.grid(True)
        plt.tight_layout()
        plt.savefig(os.path.join(plot_dir, "finetuned_a2c_policy_loss.png"))
        plt.close()
    ...
    if len(value_losses) > 0:
        ...
        plt.title("Fine-tuned A2C Value Loss")
        plt.legend()
        plt.grid(True)
        plt.tight_layout()
        plt.savefig(os.path.join(plot_dir, "finetuned_a2c_value_loss.png"))
        plt.close()

    if len(entropy_coefs) > 0:
        plt.figure()
        plt.plot(update_steps, entropy_coefs, label="Entropy Coefficient")
        plt.xlabel("Training Step")
        ...
        plt.grid(True)
        plt.tight_layout()
        plt.savefig(os.path.join(plot_dir, "finetuned_a2c_entropy_decay.png"))
        plt.close()

    if len(learning_rates) > 0:
        plt.figure()
        plt.plot(update_steps, learning_rates, label="Learning Rate")
        plt.xlabel("Training Step")
        ...
        plt.grid(True)
        plt.tight_layout()
        plt.savefig(os.path.join(plot_dir, "finetuned_a2c_lr_schedule.png"))
        plt.close()
...
def main(total_steps=TOTAL_STEPS, save_dir=DEFAULT_SAVE_DIR, seed=0):
    ...
    print("Fine-tuned A2C on CarRacing-v3")
    print("Seed:", seed)
    ...
    # Keep Adam because RMSprop was unstable in the previous result.
    optimizer = optim.Adam(model.parameters(), lr=LR)

    total_updates = max(1, total_steps // (N_STEPS * NUM_ENVS))
    ...
    while global_step < total_steps:
        ...
        # Compute advantages in REVERSE, store in forward order.
        # advantage[i] = R_i - V(s_i); grad flows ONLY through values[i].
        ...
        for i in reversed(range(rollout_len)):
            ...
            advantages_rev.append(R - values[i])

        advantages = list(reversed(advantages_rev))  # forward order

        # Normalization stats from a DETACHED copy -> no value grad leaks into stats.
        adv_flat = torch.cat([a.detach().reshape(-1) for a in advantages])
        ...
        for i in range(rollout_len):
            ...
            normalized_advantage = (advantage.detach() - adv_mean) / adv_std

            policy_loss = policy_loss - (log_probs[i].unsqueeze(1) * normalized_advantage).mean()
        ...
        if global_step - last_saved_step >= SAVE_FREQ:
            checkpoint_path = os.path.join(model_dir, f"finetuned_a2c_step_{global_step}.pt")
    ...
    final_model_path = os.path.join(model_dir, "finetuned_a2c_car_racing_final.pt")
    ...
    summary = {
        "algorithm": "Fine-tuned A2C",
        "seed": seed,
        "total_steps": total_steps,
        "num_envs": NUM_ENVS,
        "n_steps": N_STEPS,
        "optimizer": "Adam",
        "uses_lr_scheduler": True,
        "uses_entropy_decay": True,
        "uses_advantage_normalization": True,
